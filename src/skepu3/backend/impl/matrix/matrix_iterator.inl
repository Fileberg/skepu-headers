 ...
 */
 ...
 namespace skepu
 {
	 ...
	 template <typename T>
	 typename Matrix<T>::iterator MatrixIterator<T>::stridedBegin(size_t n, int s)
	 {
		 // DEBUG_TEXT_LEVEL1("MatrixIterator::stridedBegin n: " << n << ", s: " << s);
		 // std::cout << "MatrixIterator::stridedBegin n: " << n << ", s: " << s << "\n";
		 return typename Matrix<T>::iterator(this->m_parent, &this->m_std_iterator[(s < 0) ? (-n + 1) * s : 0]);
	 }
