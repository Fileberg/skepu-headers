...
namespace skepu
{
	namespace backend
	{
		...
		::mapSingleThread_CU(size_t deviceID, size_t startIdx, size_t size, size_t memSizeAlt, pack_indices<OI...>, pack_indices<EI...>, pack_indices<AI...>, pack_indices<CI...>, CallArgs&&... args)
		{
			...
			// DEBUG_TEXT_LEVEL1("CUDA Map: startIdx = " << startIdx << ", size = " << size);
			...
			auto outMemP = std::make_tuple(std::get<OI>(oArgs).getParent().updateDevice_CU(
				...
				memSizeAlt > 0 ? memSizeAlt : size * abs(this->m_strides[OI]), deviceID,
		...
		template<size_t... OI, size_t... EI, size_t... AI, size_t... CI, typename... CallArgs>
		void Map<arity, MapFunc, CUDAKernel, CLKernel>
		::CUDA(size_t startIdx, size_t size, size_t memSizeAlt, pack_indices<OI...> oi, pack_indices<EI...> ei, pack_indices<AI...> ai, pack_indices<CI...> ci, CallArgs&&... args)
		{
			...
			if (numDevices <= 1)
			{
				...
				return this->mapSingleThread_CU(this->m_environment->bestCUDADevID, startIdx, size, memSizeAlt, oi, ei, ai, ci, std::forward<CallArgs>(args)...);
			}
