...
namespace skepu
{
	...
	public:
		...
		Iterator operator()(size_t startIdx, Iterator res, Iterator res_end, CallArgs&&... args)
		{
			DEBUG_TEXT_LEVEL1("Using this one");
		...
		// template<typename... CallArgs>
		// auto operator()(size_t startIdx, CallArgs&&... args) -> decltype(get<0>(std::forward<CallArgs>(args)...))
		// {
		// 	static_assert(sizeof...(CallArgs) == numArgs-1, "Number of arguments not matching Map function");
		// 	this->apply(0,
		// 		get<0>(std::forward<CallArgs>(args)...).size() / this->m_strides[0],
